...
router.post("/vendor/register", upload.single("image"), function (req, res) {
  ...
  req.checkBody("city", "City is not valid").isAlpha();
    ...
    req.checkBody("pincode", "Pincode is not valid").matches("^[abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ][0-9][abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ] {0,1}[0-9][abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ][0-9]$");
  req.checkBody("mobile", "Mobile number can only have numbers").isNumeric({ no_symbols: true });
  req.checkBody("mobile", "Mobile number is not valid").matches("^[3-9][0-9]{9}$");
